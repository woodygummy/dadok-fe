  ...
  return (
    <div className="relative h-full min-h-0 w-full overflow-hidden">
      ...
      <Placeholder
        ...
        color={clock.color}
        round
        className="absolute top-[12%] left-[8%] z-[2] size-[4.5rem]"
      />
      <Placeholder
        name={lamp.name}
        color={lamp.color}
        className="absolute top-[5%] left-[40%] z-[3] h-20 w-[4.5rem]"
      />
      <Link
        href="/shelf"
        aria-label={`책장으로 이동, 책 ${books.length}권`}
        className="absolute right-[6%] bottom-[32%] z-[4] h-[19rem] w-[5.5rem]"
      >
        <Placeholder
          name={bookshelf.name}
      ...
      <Placeholder
        name={plant.name}
        color={plant.color}
        className="absolute bottom-[32%] left-[5%] z-[5] h-[5.75rem] w-14"
      />
      <Placeholder
        name={chair.name}
        color={chair.color}
        className="absolute bottom-[32%] left-[30%] z-[6] h-[6.5rem] w-16"
      />
      <Placeholder
        name={desk.name}
        color={desk.color}
        className="absolute bottom-[32%] left-[22%] z-[7] h-12 w-48 text-[#F6F0E2]"
      />
              ...
              {tabItems.map((item) => {
                ...
                return (
                  <button
                    ...
                    className="h-[7.5rem] w-[5.5rem] shrink-0"
                  >
                    <SketchFrame
                      className={cn(
                        "h-full rounded-[10px]",
